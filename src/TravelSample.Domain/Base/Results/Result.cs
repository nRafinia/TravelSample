namespace TravelSample.Domain.Base.Results;
...
public class Result
{
    ...
    protected Result(bool isSuccess, Error? error)
    {
        switch (isSuccess)
        {
            case true when (error is not null && error != Error.None):
                throw new InvalidOperationException();
            case false when (error is null || error == Error.None):
                throw new InvalidOperationException();
            default:
                IsSuccess = isSuccess;
                Error = error;
    ...
    /// </remarks>
    public static Result<TValue?> Failure<TValue>(Error error, TValue? value = default) =>
